...
function main() {
  ...
  let C = document.querySelector("#id-flower");
  C.width = W;
  C.height = H;
  ctx = C.getContext("2d");
  ctx.fillStyle = "#000";
...
function update(i) {
  ...
  if ((cf.dis > (H - W) * 4 && cf.dis > (W - H) * 4) || cf.gros <= -50) {
    cont = false;
  }
...
function rgb_r() {
  ...
  else if (cf.r < 10) cf.r += rr(0, 20);
  else cf.r += rr(0, 0);
}

function rgb_g() {
  if (cf.g >= 10 && cf.g <= 245) cf.g += rr(-10, 10);
  else if (cf.g < 10) cf.g += rr(0, 0);
  else cf.g += rr(-5, 0);
}

function rgb_b() {
  if (cf.b >= 10 && cf.b <= 245) cf.b += rr(-10, 10);
  else if (cf.b < 10) cf.b += rr(0, 0);
  else cf.b += rr(-5, 0);
}
